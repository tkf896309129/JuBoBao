...
import static com.example.huoshangkou.jubowan.chat.helper.CustomMessage.JSON_VERSION_3_ANDROID_IOS_TRTC;
import static com.example.huoshangkou.jubowan.chat.helper.CustomMessage.VIDEO_CALL_ACTION_ACCEPTED;
import static com.example.huoshangkou.jubowan.chat.helper.CustomMessage.VIDEO_CALL_ACTION_DIALING;
import static com.example.huoshangkou.jubowan.chat.helper.CustomMessage.VIDEO_CALL_ACTION_HANGUP;
import static com.example.huoshangkou.jubowan.chat.helper.CustomMessage.VIDEO_CALL_ACTION_LINE_BUSY;
import static com.example.huoshangkou.jubowan.chat.helper.CustomMessage.VIDEO_CALL_ACTION_REJECT;
import static com.example.huoshangkou.jubowan.chat.helper.CustomMessage.VIDEO_CALL_ACTION_SPONSOR_CANCEL;
import static com.example.huoshangkou.jubowan.chat.helper.CustomMessage.VIDEO_CALL_ACTION_SPONSOR_TIMEOUT;
...
//        mTRTCCloud.setListener(TRTCListener.getInstance());
    ...
    public void onCreate() {
        mTRTCCloud = TRTCCloud.sharedInstance(BaseApp.getInstance());
    ...
    public void onDraw(ICustomMessageViewGroup parent, CustomMessage data) {
        ...
        switch (data.action) {
            ...
            case VIDEO_CALL_ACTION_LINE_BUSY:
                ...
                break;
            default:
                LogUtils.e(TAG, "unknown data.action: " + data.action);
    ...
    public void sendVideoCallAction(int action, CustomMessage roomInfo) {
        LogUtils.i(TAG, "sendVideoCallAction action: " + action
    ...
    private String createCallID() {
        ...
        sb.append(GenerateChatUserSig.SDKAPPID).append("-").append(TIMManager.getInstance().getLoginUser()).append("-");
        for (int i = 0; i < 32; i++) {
    ...
    private void onNewComingCall(CustomMessage message) {
        LogUtils.i(TAG, "onNewComingCall current state: " + mCurrentVideoCallStatus
        ...
        switch (message.action) {
            ...
            case VIDEO_CALL_ACTION_LINE_BUSY:
                ...
                break;
            default:
                LogUtils.e(TAG, "unknown data.action: " + message.action);
                break;
        }
    }
    ...
    private void startC2CConversation(CustomMessage message) {
        ...
        Intent intent = new Intent(BaseApp.getInstance(), ChatActivity.class);
    ...
    private boolean showIncomingDialingDialog() {
        ...
        mDialog.setNegativeButton("拒绝", new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                LogUtils.i(TAG, "VIDEO_CALL_ACTION_REJECT");
                mHandler.removeCallbacksAndMessages(null);
                mCurrentVideoCallStatus = VIDEO_CALL_STATUS_FREE;
                sendVideoCallAction(VIDEO_CALL_ACTION_REJECT, mOnlineCall);
            }
